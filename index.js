...
// Usage: /movie The Matrix
bot.onText(/\/movie\s+(.+)/i, async (msg, match) => {
    const chatId = msg.chat.id.toString();
    if (chatId !== ADMIN_ID) return;

    const movieQuery = match[1].trim();
    let statusMsg = await bot.sendMessage(chatId, `[SYSTEM] Searching global movie database for: ${movieQuery}...`);

    try {
        // Hit the YTS JSON API for instantaneous results
        const response = await axios.get(`https://yts.mx/api/v2/list_movies.json?query_term=${encodeURIComponent(movieQuery)}&limit=1`);
        const data = response.data.data;

        // If the API returns nothing
        if (!data || !data.movies || data.movies.length === 0) {
            await bot.editMessageText(`[ERROR] No movies found matching "${movieQuery}". Try being more specific.`, { chat_id: chatId, message_id: statusMsg.message_id });
            return;
        }

        const movie = data.movies[0]; // Grab the #1 closest search result
        const title = movie.title_long;
        const rating = movie.rating;
        const summary = movie.summary.length > 300 ? movie.summary.substring(0, 300) + '...' : movie.summary;
        const coverImage = movie.large_cover_image || movie.medium_cover_image;
        const torrents = movie.torrents || [];

        if (torrents.length === 0) {
            await bot.editMessageText(`[ERROR] Found "${title}", but no download links are currently available.`, { chat_id: chatId, message_id: statusMsg.message_id });
            return;
        }

        // Build the Telegram Inline Keyboard Buttons and tap-to-copy Magnet Links
        const keyboard = [];
        let magnetText = `\n\n*Magnet Links (Tap to copy):*\n`;

        torrents.forEach(t => {
            // Add a clickable button for the raw .torrent file download
            keyboard.push([{ text: `Download ${t.quality} (${t.size})`, url: t.url }]);
            
            // Construct the raw Magnet Link for torrent clients
            const hash = t.hash;
            const magnet = `magnet:?xt=urn:btih:${hash}&dn=${encodeURIComponent(movie.title)}&tr=udp://open.demonii.com:1337/announce&tr=udp://tracker.openbittorrent.com:80`;
            magnetText += `\n*${t.quality}:*\n\`${magnet}\`\n`;
        });

        const caption = `*${title}*\n *IMDB:* ${rating}/10\n\n *Synopsis:*\n_${summary}_${magnetText}`;

        // Send the beautiful Movie Poster along with all the data
        ...
        await bot.editMessageText(`[ERROR] Movie search failed: ${err.message}`, { chat_id: chatId, message_id: statusMsg.message_id });
    }
});


// Usage: /music Davido Feel
bot.onText(/\/music\s+(.+)/i, async (msg, match) => {
    const chatId = msg.chat.id.toString();
    if (chatId !== ADMIN_ID) return;

    const songName = match[1].trim();
    let statusMsg = await bot.sendMessage(chatId, `[SYSTEM] Searching for audio: "${songName}"...`);
    ...
    try {
        await new Promise((resolve, reject) => {
            const cmd = `yt-dlp "ytsearch1:${songName}" --extract-audio --audio-format mp3 --audio-quality 0 --no-warnings -o "${audioPath}"`;
            
            exec(cmd, (error) => {
                if (error) reject(new Error("Song not found or download failed."));
                else resolve();
            });
        });
        ...
        if (fileSizeMB > 49.5) {
            await bot.editMessageText(`[ERROR] Audio is too large (${fileSizeMB.toFixed(1)}MB). Telegram limit is 50MB.`, { chat_id: chatId, message_id: statusMsg.message_id });
        } else {
            await bot.editMessageText('[SYSTEM] Audio found. Uploading...', { chat_id: chatId, message_id: statusMsg.message_id });
...
// Usage: /mv Burna Boy City Boys
bot.onText(/\/mv\s+(.+)/i, async (msg, match) => {
    const chatId = msg.chat.id.toString();
    if (chatId !== ADMIN_ID) return;

    const videoName = match[1].trim();
    let statusMsg = await bot.sendMessage(chatId, `[SYSTEM] Searching for video: "${videoName}"...`);
    
    const videoPath = path.join(__dirname, `mv_${Date.now()}.mp4`);
    const { exec } = require('child_process');

    try {
        await new Promise((resolve, reject) => {
            const cmd = `yt-dlp "ytsearch1:${videoName}" -f "bestvideo[height<=720][ext=mp4]+bestaudio[ext=m4a]/best[height<=720][ext=mp4]/best" --merge-output-format mp4 --no-warnings -o "${videoPath}"`;
            
            exec(cmd, (error) => {
                if (error) reject(new Error("Video not found or download failed."));
                else resolve();
            });
        });
        ...
        if (fileSizeMB > 49.5) {
            await bot.editMessageText(`[ERROR] Video exceeds 50MB limit (${fileSizeMB.toFixed(1)}MB).`, { chat_id: chatId, message_id: statusMsg.message_id });
        } else {
            await bot.editMessageText('[SYSTEM] Video found. Uploading...', { chat_id: chatId, message_id: statusMsg.message_id });
            await bot.sendVideo(chatId, videoPath);
            await bot.deleteMessage(chatId, statusMsg.message_id).catch(()=>{});
        }
    } catch (err) {
